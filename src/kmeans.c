...
#define KMEANS_MAX_GRAY_VALUE 255
...
void kmeans_uint8_histogram(const uint8_t *data, size_t length, size_t k, uint8_t *output)
{
    ...
    /* uint8 Image Histogram */
    size_t histogram_1d[256] = {0}; // gray_value -> count

    for (size_t i=0; i<length; ++i) histogram_1d[data[i]]++;

    /* gray_value -> centroid
     * 0, 1, 2, ..., k-1 are real clusters
     * k represents the gray value does not belong to any cluster. */
    size_t gray_value_to_cluster[256] = {k}; 
    ...
    while (true) {
        ...
        // assign each gray value to nearest color (uint8) cluster.
        for (size_t i=0; i<KMEANS_MAX_GRAY_VALUE+1; ++i) {
            if (histogram_1d[i] < 1) continue;

            closest_cluster = 0;
            current_min_distance = squared_distance(i, centroids[closest_cluster]);
            
            for (size_t j=1; j<k; ++j) {
                candidate_min_distance = squared_distance(i, centroids[j]);
                if (candidate_min_distance < current_min_distance) {
                    current_min_distance = candidate_min_distance;
                    closest_cluster = j;
                }
            ...
            // udpate counters: assigned cluster and cummulative sum -> weighted by histogram values.
            cluster_elements_counters[closest_cluster] += histogram_1d[i];
            cluster_elements_sums[closest_cluster] += i * histogram_1d[i];
            gray_value_to_cluster[i] = closest_cluster;
        }
    ...
    // reconstruct image with centroid value.
   for (size_t i=0; i<length; ++i) {
        output[i] = centroids[gray_value_to_cluster[data[i]]];
    }
        free(centroids);
        free(cluster_elements_counters);
        free(cluster_elements_sums);
